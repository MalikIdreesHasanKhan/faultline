...
const KineticCopy: React.FC<{children: React.ReactNode; className?: string; style?: React.CSSProperties}> = ({children, className = "", style}) => {
  ...
  return <div className={`v2-copy ${className}`} style={{...style, opacity: inValue, transform: `translateY(${(1 - inValue) * 26}px) skewX(${(1 - inValue) * -3}deg)`}}>{children}</div>;
};
...
const edgePairs: Array<[string, string]> = [["source", "features"], ["source", "clv"], ["features", "model"], ["clv", "model"], ["model", "deploy"]];
...
const OpeningScene: React.FC = () => {
  ...
  const title = smooth(frame, [265, 315]);
  return <AbsoluteFill className="v2-root"><V2Background accent={C.red} intensity={0.65} /><V2Brand />
    ...
    <KineticCopy className="v2-opening-copy" style={{opacity: title}}><strong>ONE COLUMN<br /><em>CHANGED.</em></strong></KineticCopy>
...
const TriggerScene: React.FC = () => {
  ...
  return <AbsoluteFill className="v2-root"><V2Background accent={C.teal} /><V2Brand right="CREDENTIAL-FREE JUDGE SCENARIO" />
    ...
    <Cursor x={1060 + smooth(frame, [38, 74]) * 55} y={694} clickAt={74} visible={frame > 30 && frame < 138} />
...
const EvidenceScene: React.FC = () => {
  ...
  return <AbsoluteFill className="v2-root"><V2Background accent={C.red} /><V2Brand right="02 / EVIDENCE LEDGER" />
    ...
    <Cursor x={1000} y={525} clickAt={18} visible={frame < 48} />
...
const CounterScene: React.FC = () => {
  ...
  return <AbsoluteFill className="v2-root"><V2Background accent={C.orange} /><V2Brand right="03 / COUNTERFACTUALS" />
    ...
    <Cursor x={1000} y={754} clickAt={75} visible={frame > 50 && frame < 112} /><Cursor x={720} y={754} clickAt={180} visible={frame > 168 && frame < 216} />
...
const ApprovalScene: React.FC = () => {
  const frame = useCurrentFrame();
  const typed = Math.floor(interpolate(frame, [18, 112], [0, 16], {extrapolateLeft: "clamp", extrapolateRight: "clamp"}));
  ...
  return <AbsoluteFill className="v2-root"><V2Background accent={C.lime} /><V2Brand right="04 / GOVERNANCE · APPROVAL BOUNDARY" />
    ...
    <Cursor x={1392} y={685} clickAt={166} visible={frame > 146 && frame < 190} />
...
export const FaultlineFilmV2: React.FC = () => (
  <AbsoluteFill style={{backgroundColor: C.ink}}>
    ...
    <Audio src={staticFile("audio/ambient-bed.wav")} volume={(frame) => interpolate(frame, [0, 60, V2_DURATION_SECONDS * FPS - 90, V2_DURATION_SECONDS * FPS], [0, 0.12, 0.12, 0], {extrapolateLeft: "clamp", extrapolateRight: "clamp"})} />
  </AbsoluteFill>
);
